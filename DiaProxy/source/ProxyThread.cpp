...
extern DiaServerMode diaServerMode;
...
void* _ProxyThread(void *arg)
{
	...
	if (diaServerMode == REDUNDANCY) {
	
		diaProxyState = DIAPROXY_FINDING_SERVER;

		//creating the Hearbeat Thread; it will execute the _HearbeatThread routine
		ret = pthread_create(&HearbeatThreadID,NULL,_HearbeatThread,NULL);
		...
		} while (myDiaProxyState < DIAPROXY_STARTING);
	...
	for(;;)	{ 
		...
		for (unsigned int i = 0; i < v_connections.size(); i++) {
			...
			activeTTCNConnections = 0;
                                                
			for (unsigned int i = 0; i < v_connections.size(); i++) {
				activeTTCNConnections += v_connections[i].numberOfClients;
			}
				
			if (!activeTTCNConnections && diaProxyState != DIAPROXY_STANDBY) {
                            logString.clear();
                            logString.str("");
