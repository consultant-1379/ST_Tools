        ...
        if (cer_data.latency_report_enabled){
            if (cer_data.latency_report_running){
                ...
                if (!saving_latency_data) {
                    sprintf(filename,"%s_%s_%d_%d.data",cer_data.latency_report_file,host_name,cer_data.serv_port,sample_latency_index);
         ...
         if (cer_data.DiaErrCounters_report_enabled){
            if (cer_data.DiaErrCounters_report_running){
                ...
                if (!saving_DiaErrCounters_data) {
                    sprintf(filename,"%s_absolute_%s_%d_%d.data",cer_data.DiaErrCounters_report_file,host_name,cer_data.serv_port,sample_latency_index);
	            ...
	            if (!outFile_DiaErrCounters_absolute) {
                    ...
                    sprintf(filename,"%s_percentage_%s_%d_%d.data",cer_data.DiaErrCounters_report_file,host_name,cer_data.serv_port,sample_latency_index);
                ...
                if (DiaErrCounters_report_timer >= cer_data.DiaErrCounters_report_timeout) {
...
void save_DiaErrCounters_data(ofstream & outFile_absolute, ofstream & outFile_pecentage, int time) 
{
    ...
    pthread_mutex_lock(&REPORT);
		cer_data.resultcode_request += total_ReqSent;
		cer_data.resultcode_success += total_Success;
		cer_data.resultcode_busy += total_Busy;
		cer_data.resultcode_utc += total_UnableToComply;
		cer_data.resultcode_other += total_Other;
