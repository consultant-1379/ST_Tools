...
void* _LdapServerThread(void *arg)
{
	...
	if (appStatus == CONKEEPER_HAVE_TO_EXIT){
  		if (dataTool.logMask >= LOGMASK_INFO){
       			logString.clear();
			logString.str("");
			logString << "LdapServerThread_" << myConnection->position <<": Server connection shall be closed." <<endl;
  	...
  	if (dataTool.logMask >= LOGMASK_INFO){
       		logString.clear();
		logString.str("");
		logString << "LdapServerThread_" << myConnection->position <<": Starting..." <<endl;
		LOG(INFO, logString.str());
	}
	...
	if (dataTool.logMask >= LOGMASK_DEBUG) {
       		logString.clear();
		logString.str("");
		logString << "LdapServerThread_" << myConnection->position <<": SocketId: "<< myConnection->server.sock <<", ThreadId: "<< myConnection->server.threadID<<endl;
		...
		switch (myStatus) {                        
			...
			case ONLINE:{
				...
				if(FD_ISSET(myConnection->server.sock, &tmpset)) { 
					...
					if (dataTool.logMask >= LOGMASK_DEBUG) {
 						logString.clear();
						logString.str("");
						logString << "LdapServerThread_" << myConnection->position <<": Incomming message with "<< received <<" bytes."<<endl;
						LOG(DEBUG, logString.str());
                			}
					...
					if(received < 1){
						if (dataTool.logMask >= LOGMASK_WARNING) {
 							logString.clear();
							logString.str("");
							logString << "LdapServerThread_" << myConnection->position <<": Error by reading" <<endl;
							logString <<"\tLDAP connection shall be closed" << endl;
						...
						if (dataTool.logMask >= LOGMASK_WARNING) {
 							logString.clear();
							logString.str("");
							logString << "LdapServerThread_" << myConnection->position <<": TCP connection closed by peer." <<endl;
							logString <<"\tLDAP connection shall be closed" << endl;
							LOG(WARNING, logString.str());
                				}
						resetServerConnectionAndExit (myConnection);
						...
						// some data received
							...
							pthread_mutex_unlock(&sync_mutex); 
 							...
 							if (appStatus == CONKEEPER_HAVE_TO_EXIT){
								...
								if (dataTool.logMask >= LOGMASK_INFO) {
 									logString.clear();
									logString.str("");
									logString << "LdapServerThread_" << myConnection->position <<": Received first message. It shall be stored." <<endl;
									LOG(INFO, logString.str());
                						}
							...
							int send_bytes = 0;
								...
								if (dataTool.logMask >= LOGMASK_WARNING) {
 									logString.clear();
									logString.str("");
									logString << "LdapServerThread_" << myConnection->position <<": Error sending message." <<endl;
								...
								if (dataTool.logMask >= LOGMASK_DEBUG) {
 									logString.clear();
									logString.str("");
									logString << "LdapServerThread_" << myConnection->position <<": Message sent." <<endl;
									LOG(DEBUG, logString.str());
                						}
                        				}
							...
							if (dataTool.logMask >= LOGMASK_INFO) {
 								logString.clear();
								logString.str("");
								logString << "LdapServerThread_" << myConnection->position <<": Incomming message discarded." <<endl;
								LOG(INFO, logString.str());
                					}
						}
					}
			...
			case TO_BE_CLOSED:{
				if (dataTool.logMask >= LOGMASK_INFO) {
 					logString.clear();
					logString.str("");
					logString << "LdapServerThread_" << myConnection->position <<": LDAP connection shall be closed" << endl;
					LOG(INFO, logString.str());
                		}
				resetServerConnectionAndExit (myConnection);
			...
			case TO_BE_RESTARTED:
				...
				if (dataTool.logMask >= LOGMASK_INFO) {
 					logString.clear();
					logString.str("");
					logString << "LdapServerThread_" << myConnection->position <<": Wrong connection state" << endl;
					LOG(INFO, logString.str());
                		}
                                pthread_mutex_lock(&sync_mutex);
