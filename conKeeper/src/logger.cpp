...
//	if(logmask >= what){
                switch (logmode) {
                       case  FILE_MODE:
//				pthread_mutex_lock(&lock); // Start critical section
					myFile << myUser << " (" << myPid << ") " << LOG_TYPES_MESSAGES[what] <<": " << line << std::endl;
					fileSize += line.size();
    					if (fileSize > MAX_SIZE_FILE)
        					swapFile();
//    				pthread_mutex_unlock(&lock); // End critical section
                       ...
                       case  STDOUT_MODE:
                                std::cout << myUser << " (" << myPid << ") " << LOG_TYPES_MESSAGES[what] <<": " << line << std::endl;
                                break;
...
void Log::set_log_mask(unsigned int log_mask){ 
	logmask = log_mask;
} 

void Log::set_log_mode(unsigned int log_mode){ 
	logmode = log_mode;
} 
