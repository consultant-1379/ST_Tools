...
void* _DiameterServerThread(void *arg)
{
	...
	if (appStatus == CONKEEPER_HAVE_TO_EXIT){
  		if (dataTool.logMask >= LOGMASK_INFO){
       			logString.clear();
			logString.str("");
			logString << "DiameterServerThread_" << myConnection->position <<": Server connection shall be closed." <<endl;
  	...
  	if (dataTool.logMask >= LOGMASK_INFO){
       		logString.clear();
		logString.str("");
		logString << "DiameterServerThread_" << myConnection->position <<": Starting..." <<endl;
		LOG(INFO, logString.str());
	}
	...
	if (dataTool.logMask >= LOGMASK_DEBUG) {
       		logString.clear();
		logString.str("");
		logString << "DiameterServerThread_" << myConnection->position <<": SocketId: "<< myConnection->server.sock <<", ThreadId: "<< myConnection->server.threadID<<endl;
		...
		switch (myStatus) {                        
			...
			case ONLINE:{
				...
				if(FD_ISSET(myConnection->server.sock, &tmpset)){ 
					...
					if (dataTool.logMask >= LOGMASK_DEBUG) {
 						logString.clear();
						logString.str("");
						logString << "DiameterServerThread_" << myConnection->position <<": Incomming message with "<< received <<" bytes."<<endl;
						LOG(DEBUG, logString.str());
                			}
					...
					if(received < 1){
						if (dataTool.logMask >= LOGMASK_WARNING) {
 							logString.clear();
							logString.str("");
							logString << "DiameterServerThread_" << myConnection->position <<": Error by reading" <<endl;
							logString <<"\tDIAMETER connection shall be closed" << endl;
						...
						if (dataTool.logMask >= LOGMASK_WARNING) {
 							logString.clear();
							logString.str("");
							logString << "DiameterServerThread_" << myConnection->position <<": TCP connection closed by peer." <<endl;
							logString <<"\tDIAMETER connection shall be closed" << endl;
							LOG(WARNING, logString.str());
                				}
						resetServerConnectionAndExit (myConnection);
						...
						// some data received
 							...
 							if (appStatus == CONKEEPER_HAVE_TO_EXIT){
								...
								resetServerConnectionAndExit (myConnection);
                                		...
                                		if (myConnection->client.status == ONLINE) {
                                        		if (myConnection->messageLen == 0){
								if (dataTool.logMask >= LOGMASK_INFO) {
 									logString.clear();
									logString.str("");
									logString << "DiameterServerThread_" << myConnection->position <<": Received first message. It shall be stored." <<endl;
									LOG(INFO, logString.str());
                						}
                                        			storeFirstMessage(myConnection, received, buff);
                                        		}                                        
							...
							int send_bytes = 0;
								...
								if (dataTool.logMask >= LOGMASK_WARNING) {
 									logString.clear();
									logString.str("");
									logString << "DiameterServerThread_" << myConnection->position <<": Error sending message." <<endl;
								...
								if (dataTool.logMask >= LOGMASK_DEBUG) {
 									logString.clear();
									logString.str("");
									logString << "DiameterServerThread_" << myConnection->position <<": Message sent." <<endl;
									LOG(DEBUG, logString.str());
                						}
							...
							if (dataTool.logMask >= LOGMASK_INFO) {
 								logString.clear();
								logString.str("");
								logString << "DiameterServerThread_" << myConnection->position <<": Incomming message discarded." <<endl;
								LOG(INFO, logString.str());
                					}
						}
					}                        
			...
			case TO_BE_CLOSED:{
				if (dataTool.logMask >= LOGMASK_INFO) {
 					logString.clear();
					logString.str("");
					logString << "DiameterServerThread_" << myConnection->position <<": DIAMETER connection shall be closed" << endl;
					LOG(INFO, logString.str());
                		}
				resetServerConnectionAndExit (myConnection);
			...
			case TO_BE_RESTARTED:
				...
				if (dataTool.logMask >= LOGMASK_INFO) {
 					logString.clear();
					logString.str("");
					logString << "DiameterServerThread_" << myConnection->position <<": Wrong connection state" << endl;
					LOG(INFO, logString.str());
                		}
                                pthread_mutex_lock(&sync_mutex);
...
{
	for(int i= 0; i < len; i++){
                myConnection->message[i] = buff[i];
        }
        myConnection->messageLen = len;
