		...
		if(FD_ISSET(remoteControlData.sock, &tmpset)){ 
			...
			transform( command.begin(), command.end(), command.begin(), my_tolower );
				...
				for (unsigned int i = 0; i < v_connections.size(); i++) {
					answer += get_connection_info(&v_connections[i]);
				}
                        }
...
string enable_report(std::istream& cmd)
{
    ...
    if (report == "latency") {
        dataTool.latency_report_enabled = true;
        strcpy((char*)dataTool.latency_report_file,file.c_str()); 

    }
    else if (report == "result_codes") {
        dataTool.DiaErrCounters_report_enabled = true;
        strcpy((char*)dataTool.DiaErrCounters_report_file,file.c_str()); 
    }
    else {
...
string change_result_codes_period(std::istream& cmd)
{
    ...
    dataTool.DiaErrCounters_report_timeout = seconds;			
    answer << "SUCCESS " << endl;       
    return  answer.str() ;  
...
string start_report(std::istream& cmd)
{
    ...
    if (report == "latency") {
        ...
        dataTool.latency_report_running = true;
        ...
        dataTool.DiaErrCounters_report_running = true;
    }
    else {
        answer << endl<< "ERROR: Wrong report value: "<< report << endl;
        answer << "\t Allowe values :  latency result_codes"<< endl;
...
std::string displayCmdHelp()
{
	...
	if (path == NULL) {
        ...
        help_file = help_file + "/share/DiaProxy/cnDiaProxy_help.txt";
...
string get_configuration_info() 
{
    ...
    logString << "\t\tNumber of Client Threads (max)         : "<< numberClientThreads << " (" << dataTool.maxNumberClientThreads << ")" << endl;
    logString << "\t\tDiaProxy port                          : "<< dataTool.local_port<< endl;
    logString << endl;
    
    logString <<         "\tlatency report configuration" << endl;
    logString << endl << "\t\tEnabled                        : " <<(dataTool.latency_report_enabled ? "True" : "False")<< endl;
    logString <<         "\t\tRunning                        : " <<(dataTool.latency_report_running ? "True" : "False")<< endl;
    logString <<         "\t\tFile Name Prefix               : " << dataTool.latency_report_file << endl;
    logString << endl;


    logString <<         "\tresult_codes report configuration" << endl;
    logString << endl << "\t\tEnabled                        : " <<(dataTool.DiaErrCounters_report_enabled ? "True" : "False")<< endl;
    logString <<         "\t\tRunning                        : " <<(dataTool.DiaErrCounters_report_running ? "True" : "False")<< endl;
    logString <<         "\t\tFile Name Prefix               : " << dataTool.DiaErrCounters_report_file << endl;
    logString <<         "\t\tSaving period                  : " << dataTool.DiaErrCounters_report_timeout << endl;
    logString << endl;
...
string get_result_code_info()
{
    ...
    logString << "request=" << dataTool.resultcode_request;
    logString << " success=" << dataTool.resultcode_success;
    logString << " busy=" << dataTool.resultcode_busy;
    logString << " utc=" << dataTool.resultcode_utc;
    logString << " other=" << dataTool.resultcode_other;
...
string get_connection_info(void *connection)
{
    ...
    logString << endl << "\t-------------------------------------" << endl;
    logString <<         "\tCONNECTION NUMBER " << myConnection->pos <<  endl;
    logString <<         "\t-------------------------------------" << endl;

    logString <<         "\t\tThread Id                           " << myConnection->threadID << endl;
    logString <<         "\t\tSocket Id                           " << myConnection->net_con->get_fd() << endl;

    logString <<         "\t\tDiameter server IP                  " << myConnection->diameter_host << endl;
    logString <<         "\t\tDiameter server port                " << myConnection->serv_port << endl;
    logString <<         "\t\tDiaProxy IP                         " << myConnection->diaproxy_host << endl;
    logString <<         "\t\tProtocol                            " << (myConnection->use_sctp ? "SCTP" : "TCP") << endl;
    logString <<         "\t\tOrigin host                         " << myConnection->origin_host << endl;
    logString <<         "\t\tOrigin realm                        " << myConnection->origin_realm << endl;
    logString <<         "\t\tDiameter appid                      " ;
    for (int i=0; i< myConnection->vendor_specific_application.size();i++){
        logString << myConnection->vendor_specific_application[i] << " ";
    }
    logString << endl;
