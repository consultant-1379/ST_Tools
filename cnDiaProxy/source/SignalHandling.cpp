    ...
    for(;;){
        ...
        switch (signal) {   
            ...
            case SIGTERM:
                ...
                if (dataTool.latency_report_running) {dataTool.latency_report_running = false;} 
                if (dataTool.DiaErrCounters_report_running) {dataTool.DiaErrCounters_report_running = false;} 
...
//                close_server_socket();
                ...
                if (myDiaProxyState == DIAPROXY_PROCESSING ) {
                    ...
                    for (unsigned int i = 0; i < v_connections.size(); i++) {
                        logInfo += get_connection_info(&v_connections[i]);
