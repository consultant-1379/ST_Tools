...
if [ -n "$services_to_migrate" ]; then
    ...
    /usr/bin/systemctl preset rtc_build_handler.service >/dev/null 2>&1 || :
...
if [ -f /var/run/enable_rtc_build_handler_service ]; then
    /usr/bin/systemctl --quiet enable rtc_build_handler
fi
