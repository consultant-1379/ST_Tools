...
for service in rtc_controller.service ; do
...
if [ -n "$services_to_migrate" ]; then
    ...
    /usr/bin/systemctl preset rtc_controller.service >/dev/null 2>&1 || :
...
if [ -f /var/run/enable_rtc_controller_service ]; then
    /usr/bin/systemctl --quiet enable rtc_controller
fi
